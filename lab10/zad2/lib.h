...
void sendMessage (struct message* my_message, int socket, int id, const struct sockaddr_in *dest_addr) {

    if (sendto(socket, my_message, sizeof(struct message), 0, (const struct sockaddr*) dest_addr, sizeof(struct sockaddr_in)) == -1) {
        if (id == SERVER)

...
void orderStop () {
    ...
    sendMessage(&my_message, my_socket, my_id, &my_sockaddr);

    if (recvfrom(my_socket, &my_message, sizeof(struct message), 0, NULL, NULL) == -1) {
...
int main (int argc, char** argv) {
    ...
    while (1) {
        ...
        if (recvfrom(my_socket, &my_message, sizeof(struct message), MSG_DONTWAIT, NULL, NULL) != -1) {    // if I've got something
            char my_time[35];
